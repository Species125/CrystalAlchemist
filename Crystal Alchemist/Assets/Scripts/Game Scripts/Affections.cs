...
namespace CrystalAlchemist
{
    ...
    public class Affections : ScriptableObject
    {
        ...
        public bool IsDeadAffected(Character sender, Collider2D hittedCharacter)
        {
            Character target = hittedCharacter.GetComponent<Character>();
        ...
        {
            //if (sender.IsGuestPlayer() && !byOtherPlayers) return false; //No hits from guest players allowed

            if (other)
            {
                if (sender == null) return true; 
                if (sender.IsGuestPlayer()) return false; //No hits from guest players allowed
